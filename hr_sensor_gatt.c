...
static void gattAdvertTimerHandler(timer_id tid)
{
    ...
    else if(g_pos_sensor_data.app_tid == tid)
    {
        ...
        switch(g_pos_sensor_data.state)
        {
            case app_state_fast_advertising:
            {
                /*if((g_pos_sensor_data.enable_white_list == TRUE))
                {
                    g_pos_sensor_data.advert_timer_value =
                                   FAST_CONNECTION_ADVERT_TIMEOUT_VALUE - 
                                   BONDED_DEVICE_ADVERT_TIMEOUT_VALUE;
                }
                else
...
extern void HandleAccessRead(GATT_ACCESS_IND_T *p_ind)
{
    ...
    else if(Pos_SensorCheckHandleRange(p_ind->handle))
    {
        /*JDP Attribute handle belongs to Position sensor service */
        Pos_SensorHandleAccessRead(p_ind);
    }
    else
    {
...
extern void HandleAccessWrite(GATT_ACCESS_IND_T *p_ind)
{
    ...
    {
        /*JDP add in sensor voltage reading here for left and right*/
        Pos_SensorHandleAccessWrite(p_ind);
    }        
    else
    {
...
extern void GattStartAdverts(bool fast_connection)
{
    ...
    if(g_hr_data.advert_timer_value)
    {
        ...
        g_hr_data.app_tid = TimerCreate(g_hr_data.advert_timer_value, TRUE, 
                                        gattAdvertTimerHandler);
        /*JDP added*/
        g_pos_sensor_data.app_tid = TimerCreate(g_pos_sensor_data.advert_timer_value,
...
extern uint16 GetSupported16BitUUIDServiceList(uint8 *p_service_uuid_ad)
{
    ...
    /*JDP added*/
    p_service_uuid_ad[i++] = LE8_L(UUID_POS_SENSOR_SERVICE);
    p_service_uuid_ad[i++] = LE8_H(UUID_POS_SENSOR_SERVICE);
