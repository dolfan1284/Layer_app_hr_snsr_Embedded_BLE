...
#define MAX_APP_TIMERS                 (12)   /*JDP, may want to up it from 6*/
...
static void hrSensorDataInit(void)
{
    ...
    /*Position sensor data initialization*/
    Pos_SensorDataInit();
...
static void readPersistentStore(void)
{
    ...
    /*JDP add in POS Sensor data read*/
      Pos_SensorReadDataFromNVM(&nvm_offset);
...
{
    /*JDP not used during normal operation*/
    /*Handling signal as per current state */
    switch(g_hr_data.state)
    {
        case app_state_fast_advertising:
...
static void handleSignalLsRadioEventInd(void)
{
    ...
    switch(g_hr_data.state)
    {
        case app_state_connected:
        {
            ...
            /* Delete the already running Position Sensor timer and start a new 
             * one.
             */
            TimerDelete(g_pos_sensor_data.pos_meas_tid);
            g_pos_sensor_data.pos_meas_tid =
                     TimerCreate((POS_MEAS_TIME - (5* MILLISECOND)- timer_value),
                                 TRUE, 
                                 posMeasTimerHandler);
...
{
    /*JDP, as expected, only on disconnection from BLE*/
    /* Reset the connection parameter variables. */
    g_hr_data.conn_interval = 0;
    g_hr_data.conn_latency = 0;
    g_hr_data.conn_timeout = 0;
...
extern void AppInit(sleep_state last_sleep_state)
{
    ...
    /*JDP added for UART func*/
    /* Reassign PIO9 to UART receive */
    PioSetMode(9, pio_mode_uart);
    
    /* Reassign PIO10 to UART transmit */
    PioSetMode(10, pio_mode_uart);
    ...
    /*Init Pos Sensor State, say we are connected*/
    g_pos_sensor_data.state = app_state_connected;
